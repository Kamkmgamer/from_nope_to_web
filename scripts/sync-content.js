...
const COURSE_CONFIG = {
  ...
  "01-Web-Foundations": {
    ...
    descriptionEn:
      "Your first step into web development. Master the structure and style of the internet.",
  ...
  "03-React-NextJS": {
    ...
    descriptionEn:
      "Build full-stack applications with the world's most popular framework.",
  ...
  "04-T3-Stack": {
    ...
    descriptionEn:
      "Master the modern full-stack: TypeScript, Tailwind, tRPC, and Next.js",
    descriptionAr:
      "أتقن التطوير المتكامل الحديث: TypeScript, Tailwind, tRPC, و Next.js",
    image:
...
function extractMetadata(content) {
  ...
  const timeMatch =
    content.match(/Estimated Time:?\*\*?\s*~?(\d+)/i) ||
    content.match(/Duration:?\*\*?\s*~?(\d+)/i) ||
    content.match(/Time:?\s*\|?\s*(\d+)/i); // Covers table format | 15 min |

  return {
    title:
      titleMatch && titleMatch[1] ? titleMatch[1].trim() : "Untitled Lesson",
    time: timeMatch && timeMatch[1] ? parseInt(timeMatch[1]) : 15,
  };
}
...
function processDirectory() {
  ...
  const courseDirs = fs
    .readdirSync(EN_DIR)
    .filter((f) => fs.statSync(path.join(EN_DIR, f)).isDirectory());

  /** @type {typeof COURSE_CONFIG} */
  const courseConfig = COURSE_CONFIG;

  for (const dirName of courseDirs) {
    if (!courseConfig[/** @type {keyof typeof courseConfig} */ (dirName)])
      continue;

    const config =
      courseConfig[/** @type {keyof typeof courseConfig} */ (dirName)];
    const coursePathEn = path.join(EN_DIR, dirName);
    const coursePathAr = path.join(AR_DIR, dirName);

    const lessons = [];
    const files = fs
      .readdirSync(coursePathEn)
      .filter((f) => f.endsWith(".md") && f !== "README.md");

    for (const fileName of files) {
      const contentEn = fs.readFileSync(
        path.join(coursePathEn, fileName),
        "utf-8",
      );
      let contentAr = "";
  ...
  console.log(
    `Successfully generated seedData.ts with ${courses.length} courses!`,
  );
}
